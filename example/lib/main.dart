...
class MyApp extends StatefulWidget {
  const MyApp({
    super.key,
    required this.bluesnapSdkPlugin,
    required this.assetService,
    required this.apiService,
  });
  final BluesnapSdk bluesnapSdkPlugin;
...
class _MyAppState extends State<MyApp> {
  // final _dio = Dio();
  late BluesnapSdk bluesnapSdkPlugin;
  String? _result;

  @override
  void initState() {
    bluesnapSdkPlugin = widget.bluesnapSdkPlugin;

    ///listen onGenerateToken in bluesnap native sdk
    bluesnapSdkPlugin.onGenerateToken.listen((data) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      try {
        await bluesnapSdkPlugin.initBluesnap(
          bsToken: 'bsToken',
          initKount: true,
          fraudSessionId: '',
  ...
  Future<void> _showCheckout() async {
    ...
    try {
      await bluesnapSdkPlugin.setSDKRequest(
        withEmail: false,
        withShipping: false,
        fullBilling: false,
        amount: 2.0,
      ...
      final result = await bluesnapSdkPlugin.showCheckout();
  ...
  Future<void> _checkout(
    ...
    try {
      ...
      final result = await bluesnapSdkPlugin.checkoutCard(
        cardNumber: cardNumber.isEmpty ? '4242424242424242' : cardNumber,
        expirationDate: expirationDate.isEmpty ? '01/25' : expirationDate,
  ...
  Future<Map<String, dynamic>> readCredentials() async {
    ...
    String jsonString =
        await widget.assetService.loadString('assets/credentials.json');
